...
def export_camera_to_usd(
    ...
    script_path = SCRIPTS_DIR / "export_camera_usd.py"
    if not script_path.exists():
        raise FileNotFoundError(f"Export script not found: {script_path}")
    ...
    print(f"Running Blender headless USD camera export...")
    print(f"  Input: {camera_dir}")
    print(f"  Output: {output_path}")
    print(f"  FPS: {fps}")
    print(f"  Start frame: {start_frame}")
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            print(f"stdout: {stdout}")
            print(f"stderr: {stderr}")
            raise RuntimeError(f"Blender USD camera export failed: {stderr}")
