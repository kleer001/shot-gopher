#!/usr/bin/env python3
"""Blender script to export animated mesh + camera to a single USD file.

Combines OBJ mesh sequence and camera JSON data into one .usd containing
both an animated mesh and an animated camera node.

Usage (from command line):
    blender -b --python export_scene_usd.py -- \
...
def export_usd(output_path: Path, start_frame: int, end_frame: int, fps: int):
    """Export entire scene to USD."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Export mesh + camera to single USD"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output", "-o", type=Path, required=True, help="Output USD file path"
    )
    ...
    print(f"\nExporting combined scene USD...")
    print(f"  Output: {args.output}")
    export_usd(args.output, args.start_frame, end_frame, args.fps)
