...
class ConfigurationGenerator:
    ...
    def _get_tool_bin_dirs(self) -> List[Path]:
        """Find bin directories under .vfx_pipeline/tools/*/."""
    ...
    def generate_activation_script(self) -> str:
        ...
        """
        tool_dirs = self._get_tool_bin_dirs()
        path_additions = ":".join(str(d) for d in tool_dirs)
        path_export = f'\nexport PATH="{path_additions}:$PATH"' if path_additions else ""
...
"""
    ...
    def generate_activation_script_powershell(self) -> str:
        ...
        tool_dirs = self._get_tool_bin_dirs()
        path_lines = "\n".join(
            f'$env:PATH = "{str(d).replace("/", chr(92))};$env:PATH"'
            for d in tool_dirs
        )
...
"""
    ...
    def generate_activation_script_batch(self) -> str:
        ...
        tool_dirs = self._get_tool_bin_dirs()
        path_lines = "\n".join(
            f'set "PATH={str(d).replace("/", chr(92))};%PATH%"'
            for d in tool_dirs
        )
...
call {self.conda_manager.get_activation_command()}

REM Add installed tools to PATH
{path_lines}
