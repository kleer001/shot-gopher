...
def render_set(
    ...
    c2w = torch.inverse(ref_view.world_view_transform.T)
    view_dirs_ = (
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        if indirect:
            ...
            occlusion = torch.ones_like(depth_map).permute(1, 2, 0)
            irradiance = torch.zeros_like(depth_map).permute(1, 2, 0)
...
def eval_brdf(data_root: str, scene: Scene, model_path: str, name: str) -> None:
    ...
    for idx, frame in enumerate(tqdm(frames)):
        ...
        albedo_gt = torch.from_numpy(albedo_gt).cuda() / 255.0
        ...
        albedo_map = brdf_map[:, : (W3 // 3), :]
        albedo_map = torch.from_numpy(albedo_map).cuda() / 255.0
